...
const machineClassName =
  "relative isolate w-full overflow-hidden rounded-[var(--printer-radius)] border border-sage/40 bg-[color-mix(in_oklab,var(--sage)_18%,var(--cream))] p-[var(--printer-inset)] pb-8 shadow-[0_20px_36px_-20px_color-mix(in_oklab,var(--sage)_40%,transparent),0_6px_14px_-8px_color-mix(in_oklab,var(--foreground)_16%,transparent),inset_0_1px_0_color-mix(in_oklab,white_55%,transparent),inset_0_-1px_0_color-mix(in_oklab,var(--sage)_35%,transparent)] [--printer-inner-radius:calc(var(--printer-radius)_-_var(--printer-inset))] [--printer-inset:0.75rem] [--printer-radius:1.5rem] before:pointer-events-none before:absolute before:inset-0 before:z-0 before:rounded-[inherit] before:bg-[url('/textures/plastic-noise.svg')] before:bg-[length:180px_180px] before:bg-repeat before:opacity-40 before:mix-blend-multiply before:content-['']";
...
function ReceiptPrinterMachine({
  ...
  return (
    <div className={cn(machineClassName, className)} {...props}>
      ...
      <div
        aria-hidden="true"
        className="absolute inset-x-6 bottom-[var(--printer-inset)] z-40 h-2 rounded-[0.25rem] border border-sage/50 bg-sage shadow-inner"
      />
    </div>
  );
}
...
function ReceiptPrinterScreen({
  ...
  return (
    <div
      className={cn(
        "relative z-10 isolate overflow-hidden rounded-[var(--printer-inner-radius)] border border-sage/60 bg-sage p-4 text-cream shadow-[inset_0_2px_10px_color-mix(in_oklab,black_35%,transparent)] after:pointer-events-none after:absolute after:inset-0 after:z-20 after:rounded-[inherit] after:shadow-[inset_0_0_24px_4px_color-mix(in_oklab,black_40%,transparent)] after:content-['']",
        className,
      )}
      {...props}
    >
      ...
      <AnimatePresence initial={false} mode="sync">
        {isComplete ? (
          <motion.span
            ...
            className="col-start-1 row-start-1 grid place-items-center text-sale"
            exit={{
              opacity: animate ? 0 : 1,
              scale: move ? 0.96 : 1,
            }}
          ...
          >
            <CheckCircle2 className="size-[18px]" strokeWidth={2.25} />
          </motion.span>
        ) : (
          <motion.span
            animate={{ opacity: 1, scale: 1 }}
            className="col-start-1 row-start-1 grid place-items-center text-cream/70"
            exit={{
              opacity: animate ? 0 : 1,
              scale: move ? 0.96 : 1,
            }}
      ...
      >
        <AnimatePresence initial={false} mode="sync">
          <motion.div
            ...
            className="col-start-1 row-start-1 truncate font-medium text-xs leading-none text-cream/85"
            exit={{
              opacity: animate ? 0 : 1,
              y: shouldMove ? -4 : 0,
            }}
...
function ReceiptPrinterPaper({
  ...
  return (
    <article
      className={cn(
        "relative z-10 min-h-80 bg-cream bg-[url('/textures/receipt-paper.svg')] bg-cover px-6 pt-7 pb-8 font-mono text-foreground bg-blend-soft-light",
        className,
      )}
...
function ReceiptPrinterOutput({
  ...
  return (
    <div
      className={cn(
        "relative z-10 -mt-4 h-[32rem] w-[calc(80%+3rem)] max-w-full overflow-hidden px-6",
        className,
      )}
      {...props}
    >
      {isReceiptVisible ? (
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-6 -top-1 z-20 h-2 bg-sage/70 blur-[6px]"
        />
      ) : null}
      ...
      <motion.div
        ...
        className="relative isolate before:pointer-events-none before:absolute before:inset-x-3 before:top-3 before:bottom-4 before:z-0 before:rounded-sm before:shadow-[0_8px_24px_color-mix(in_oklab,var(--sage)_22%,transparent)] before:content-[''] after:pointer-events-none after:absolute after:right-[8%] after:bottom-0 after:left-[8%] after:z-0 after:h-3 after:translate-y-1.5 after:rounded-full after:bg-sage/10 after:blur-lg after:content-['']"
        initial={false}
        transition={{
